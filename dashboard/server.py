...
def _json_response(handler, data: Any, status: int = 200, extra_headers: list[tuple[str, str]] = None):
...
class DashboardHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def _gh_headers(self) -> dict[str, str]:
        ...
        if cookie_header:
            ...
            if "github_token" in C:
                cookie_token = C["github_token"].value
    ...
    def do_GET(self):
        ...
        elif path == "/api/github/login":
            self._serve_github_login(params)
        elif path == "/api/github/logout":
            self.send_response(302)
            self.send_header('Location', "/")
            self.send_header('Set-Cookie', "github_token=; Path=/; HttpOnly; SameSite=Lax; Max-Age=0")
            self.end_headers()
        elif path == "/api/github/callback":
            self._serve_github_callback(params)
        elif path == "/":
    ...
    def _serve_github_login(self, params):
        client_id = os.getenv("GITHUB_CLIENT_ID")
        if not client_id:
            _json_response(self, {"error": "GITHUB_CLIENT_ID not set"}, status=500)
            return
        redirect_url = f"https://github.com/login/oauth/authorize?client_id={client_id}&scope=repo"
        self.send_response(302)
        self.send_header('Location', redirect_url)
        self.end_headers()

    def _serve_github_callback(self, params):
        code = params.get("code", [None])[0]
        if not code:
            _json_response(self, {"error": "code query param required"}, status=400)
            return

        client_id = os.getenv("GITHUB_CLIENT_ID")
        client_secret = os.getenv("GITHUB_CLIENT_SECRET")
        if not client_id or not client_secret:
            _json_response(self, {"error": "GITHUB_CLIENT_ID or GITHUB_CLIENT_SECRET not set"}, status=500)
            return
        ...
        if resp.status_code == 200:
            ...
            if token:
                ...
                self.send_response(302)
                self.send_header('Location', "/")
                self.send_header('Set-Cookie', f"github_token={token}; Path=/; HttpOnly; SameSite=Lax")
                self.end_headers()
                return
        
        # fallback
        _json_response(self, {"error": "oauth failed", "details": resp.text}, status=400)
    ...
    def _serve_github_status(self):
        ...
        try:
            resp = _requests.get(f"{GITHUB_API}/user", headers=self._gh_headers(), timeout=10)
            if resp.status_code == 200:
                user = resp.json()
                result = {
                    "connected": True,
    ...
    def _serve_github_user(self):
        ...
        try:
            ...
            _json_response(self, {"error": "Internal Server Error"}, status=500)
    ...
    def _run_review(self):
        ...
        try:
            ...
            if not repo or not pr_number:
                _json_response(self, {"error": "repo and pr_number are required"}, status=400)
                return
    ...
    def _merge_pr(self):
        ...
        try:
            ...
            if merge_resp.status_code == 200:
                ...
                if f"gh_prs_{repo}" in _cache:
                    _cache.pop(f"gh_prs_{repo}")
                _json_response(self, {
                    "status": "merged",
                    "sha": result.get("sha", ""),
    ...
    def _apply_fix(self):
        ...
        try:
            ...
            pr_resp = _requests.get(f"{GITHUB_API}/repos/{repo}/pulls/{pr_number}", headers=self._gh_headers())
            if pr_resp.status_code != 200:
            ...
            if original_content and original_content in content:
                new_content = content.replace(original_content, fix_code, 1)
            else:
                ...
                if 0 <= line_idx < len(lines):
                    ...
                    _json_response(self, {"error": "Line number out of bounds or match failed"}, status=400)
                    return
